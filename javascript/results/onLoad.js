...
function loadPathwayObjs() {
	...
	for (let pathwayDiv of document.querySelectorAll(".pathway_div")){
		let pathwayObj = new Pathway({
			id: pathwayDiv.id,
			name: pathwayDiv.name
		});
		pathwayObj.loadComplexes();
		pathwayObjs[pathwayDiv.id] = pathwayObj;
	}
	console.log('PathwayObjs Loaded');
	console.log(pathwayObjs);
}
...
document.addEventListener('DOMContentLoaded', function() {
	...
	if (document.getElementById('add_element_to_logic_button')) {
		document.getElementById('add_element_to_logic_button').addEventListener("click", function(){
			let optionType;
			let optionValue;
			["gene","meta","urna","prot","chroma","tf"].forEach((dataType) => {
				if (selectorInterface.selectedValues.type_for_logical.value === dataType) {
					optionValue = selectorInterface.selectedValues[`${dataType}IDSelect`].value;
					optionType = dataType;
				}
			});
