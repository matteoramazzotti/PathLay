...
sub prepareEnablersForFrontEnd {
	...
	# if ($parameters -> {_nodeg_select_urna} || $parameters -> {_nodeg_select_meth} || $parameters -> {_nodeg_select_chroma}) {
	#     push(@{$script_section -> {_variables}},"var enable_nodeg_gene=1;");
	# } else {
	#     push(@{$script_section -> {_variables}},"var enable_nodeg_gene=0;");
	# }
...
package Parameters;
	...
	sub LoadENVFromJson {
		...
		my $json = $args{JSON};
		foreach my $param (sort keys %$json) {
			$self->{"_$param"} = $json->{$param};
		}
		# $self -> {_maps_db_select} = $self ->{_mapDBSelected};
