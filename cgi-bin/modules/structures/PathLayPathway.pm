...
package Pathway;
	...
	sub LoadComplexes2 {
		...
		foreach my $coord (keys %{$self -> {_nodes}}) {
			my $complex = new Complex(
				_id => $self -> {_id}."_$count",
				_coordinates => $coord,
				_index => $self -> {_index}
			...
			$complex -> CheckOffSetAndTrim(
				Nodes => $self -> {_nodes},
				Type => "deg", # this should be fixed, only one trim is useful
				Coord => $coord
			);
			$complex->InitLegends(
				MapName => $self -> {_name},
			...
			foreach my $nodeObj (@{$self->{_nodes}->{$coord}}) {

				my $i = 0;
				my @typesOnNode = $nodeObj->{type} ne "deg+prot" ? ($nodeObj->{type}) : ("prot","deg");
				my @devsOnNode = $nodeObj->{type} ne "deg+prot" ? ($nodeObj->{dev}) : ($nodeObj->{prot_dev},$nodeObj->{dev});

				foreach my $typeOnNode (@typesOnNode) {
					$seen->{$typeOnNode}->{$nodeObj->{id}} = {};
					# if ($nodeObj->{id} eq "7532") {
					# 	print STDERR Dumper  $nodeObj;
					# }
					$complex->mainLegendHandler2(
						ID => $typeOnNode eq "prot" ? $nodeObj -> {_prot_id} : $nodeObj->{id},
						mainName => $typeOnNode eq "prot" ? $nodeObj -> {id} : $nodeObj->{name},
						fullName => $typeOnNode eq "prot" ? $nodeObj -> {_prot_name} : undef,
						mainType => $typeOnNode
					);
					if ($devsOnNode[$i]) {
						$complex->effectSizeLegendHandler2(
							effectSize => $devsOnNode[$i]
						);
					} else {
						$complex -> {_queryForPlot} =~ s/$/%0A/;
						$complex -> {_legend_for_title} =~ s/$/\n/;
					}

					if ((scalar(@typesOnNode) == 1 || $i == 1 )) {
						foreach my $mod (("meth","chroma")) {
							if ($nodeObj->{$mod}) {
								$seen->{$mod}->{$nodeObj->{id}} = {};
								$complex->methAndChromaHandler(
									ID => $typeOnNode eq "prot" ? $nodeObj -> {_prot_id} : $nodeObj->{id},
									effectSize => $nodeObj->{$mod},
									Type => $mod
								);
							}
						}
						if ($nodeObj->{urnas}) {
							foreach my $urnaID (keys %{$nodeObj->{urnas}}) {
								$seen->{urna}->{$urnaID} = {};
								$complex->urnaHandler2(
									ID => $urnaID,
									MIRT => $nodeObj->{urnas}->{$urnaID}->{mirt},
									EffectSize => $nodeObj->{urnas}->{$urnaID}->{dev}   
								);
							}
						}
						if ($nodeObj->{tfs}) {
							foreach my $tfID (keys %{$nodeObj->{tfs}}) {
								$seen->{tf}->{$tfID} = {};
								$complex->tfHandler2(
									ID => $tfID,
									Name => $nodeObj->{tfs}->{$tfID}->{name},
									EffectSize => $nodeObj->{tfs}->{$tfID}->{dev}
								);
							}
						}
					}
		...
		$self -> {_tf_loaded} = scalar keys %{$seen -> {tf}};
	...
	sub PathwayLoader2 {
		...
		foreach (sort @nodes) {
			...
			next if (
				!$expPacks -> {gene} -> {_data} -> {$node_id} &&
				!$expPacks -> {nodeg} -> {_data} -> {$node_id} &&
				!$expPacks -> {meta} -> {_data} -> {$node_id} &&
				!$expPacks -> {prot} -> {_data} -> {$node_id}
			);
			foreach my $dataType (("gene","prot","nodeg","meta")) {
				if ($expPacks -> {$dataType} -> {_data} -> {$node_id}) {

					my $nodeObj = $expPacks -> {$dataType} -> {_data} -> {$node_id};
					$nodeObj->{id} = $node_id;
					$nodeObj->{name} = $node_name;
					$nodeObj->{type} = $dataType;
					
					foreach my $coord (@coords) {
						push(@{$tmpNodes->{_nodes}->{$coord}},$nodeObj);
					}
				}
			}
		}
		# check for multi type (deg/gene+prot), also assign type deg instead of gene, fix this pls
		
		foreach my $coord (keys %{$tmpNodes->{_nodes}}) {
			my $checkForMulti = {};

			foreach my $nodeObj (@{$tmpNodes->{_nodes}->{$coord}}) {
				
				if ($nodeObj->{type} eq "gene" || $nodeObj->{type} eq "prot") {
					$checkForMulti->{seen}->{$nodeObj->{id}}++;
					$checkForMulti->{$nodeObj->{type}}->{$nodeObj->{id}} = $nodeObj;
				} else {
					push(@{$checkForMulti->{nodes}},$nodeObj);
				}
				
				if ($checkForMulti->{seen}->{$nodeObj->{id}} && $checkForMulti->{seen}->{$nodeObj->{id}} > 1) {
					my $newObj = { %$nodeObj };
					$newObj -> {type} = "deg+prot";
					$newObj -> {id} = $checkForMulti->{gene}->{$nodeObj->{id}}->{id};
					$newObj -> {name} = $checkForMulti->{gene}->{$nodeObj->{id}}->{name};
					$newObj -> {dev} = $checkForMulti->{gene}->{$nodeObj->{id}}->{dev};
					$newObj -> {prot_id} = $checkForMulti->{prot}->{$nodeObj->{id}}->{_prot_id};
					$newObj -> {prot_name} = $checkForMulti->{prot}->{$nodeObj->{id}}->{_prot_name};
					$newObj -> {prot_dev} = $checkForMulti->{prot}->{$nodeObj->{id}}->{dev};

					
					delete($checkForMulti->{prot}->{$nodeObj->{id}});
					delete($checkForMulti->{gene}->{$nodeObj->{id}});
					push(@{$checkForMulti->{nodes}},$newObj);
				}
			}

			# retrieve single nodes remained
			foreach my $t (("gene","prot")) {
				foreach my $nodeID (keys %{$checkForMulti->{$t}}) {
					if ($checkForMulti->{seen}->{$nodeID} == 1) {
						if ($t eq "gene") { # this should be removed and kept as gene
							$checkForMulti->{$t}->{$nodeID}->{type} = "deg";
						}
						push(@{$checkForMulti->{nodes}},$checkForMulti->{$t}->{$nodeID});
					}
				}
			}
			#deposit new and definitive array of nodes
			@{$self -> {_nodes} -> {$coord}} = @{$checkForMulti->{nodes}};
		}
	}
	...
	sub PathwayLoader {
		...
		$self -> {_degs_loaded} = scalar keys %{$seen -> {gene}};
		$self -> {_nodegs_loaded} = scalar keys %{$seen -> {nodeg}};
		$self -> {_urnas_loaded} = scalar keys %{$seen -> {urna}};
		$self -> {_metas_loaded} = scalar keys %{$seen -> {meta}};
		$self -> {_methyls_loaded} = scalar keys %{$seen -> {meth}};
	...
	sub PathwayEncoder2 {
		...
		foreach my $complexObj (@{$self -> {_complexes}}) {
			# print STDERR $complexType."\n";
			my @elements = split("%0A",$complexObj -> {_queryForPlot});
			shift(@elements) for (1..2);
			#print STDERR Dumper \@elements;
...
package MapDiv;
	...
	sub LoadComplexesOnDiv2 {
		...
		my $pathway = $args{Pathway};
		foreach my $complex (@{$pathway -> {_complexes}}) {
			my $compid = $complex -> {_id};
			my ($map_id,$complex_number) = split(/_/,$compid);
			my $div_name = "";
			my $div_title = $complex -> {_legend_for_title};
